...
export const GET: RequestHandler = async ({ params, fetch }) => {
    ...
    try {
        ...
        // Find the specific item in the mapping
        const itemMapping = mapping.find((m) => m.id === itemId);

        if (!itemMapping) {
            return new Response('Item not found', { status: 404 });
        }

        const l = latestMap[String(itemId)];
        ...
        const volEntry = basicVolumeMap[String(itemId)];
        ...
        const timeseriesData = await getTimeseriesData(itemId);
        ...
        const postTaxProfit = calculatePostTaxProfit(high, low, itemId);
        const potentialProfit =
            (itemMapping.limit ?? 0) > 0 && postTaxProfit !== null ? (itemMapping.limit ?? 0) * postTaxProfit : null;

        const item: PriceRow = {
            id: itemMapping.id,
            name: itemMapping.name,
            // Deterministic hashed upload path to avoid client-side redirects
            icon: buildWikiImageUrl(itemMapping.icon),
            members: itemMapping.members,
            buyLimit: itemMapping.limit ?? null,
            buyPrice: high,
            ...
            wikiUrl: `https://oldschool.runescape.wiki/w/${encodeURIComponent(itemMapping.name)}`,
            highalch: itemMapping.highalch ?? null,
            lowalch: itemMapping.lowalch ?? null,
            value: itemMapping.value ?? null
        };
